...
public class KollisionPruefer {
	...
	public boolean pruefeFeld(Entity entity) {
		...
		System.out.println(entity.getClass().getSimpleName() +"  X: "+ hitBoxRechteSpalte +"  Y: "+ hitBoxObereReihe);
	...
	public boolean pruefeFeldAndOnlyReturn(Entity entity, String richtung, int bewegung) {
	...
	public boolean[] pruefeEntity(Entity entity, boolean kannInteragieren, String pruefRichtung) {
		boolean entGetroffen[] = new boolean[gp.entity.length];
		...
		for (int i = 0; i < entGetroffen.length; i++) {
			if (gp.entity[i] != null) {
				...
				// Alte Werte Sichern
				int altesEntity2HitboxX =  gp.entity[i].hitBox.x;
				int altesEntity2HitboxY =  gp.entity[i].hitBox.y;
				// Hitbox Weltposition bestimmen
				gp.entity[i].hitBox.x = gp.entity[i].weltX + gp.entity[i].hitBox.x;
				gp.entity[i].hitBox.y = gp.entity[i].weltY + gp.entity[i].hitBox.y;
				
				switch (pruefRichtung) {
				case "oben":
					entity.hitBox.y -= entity.geschwindigkeit;
					if (entity.hitBox.intersects(gp.entity[i].hitBox)) {
						if (gp.entity[i].kollision==true) {
							entity.kollidiert = true;
						}
						if (kannInteragieren) {
							entGetroffen[i] = true;
				...
				gp.entity[i].hitBox.x = altesEntity2HitboxX;
				gp.entity[i].hitBox.y = altesEntity2HitboxY;
			} else {
				if (kannInteragieren) {
					entGetroffen[i] = false;
				}
